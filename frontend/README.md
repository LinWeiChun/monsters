...
登入流程只顯示與送出 Email，使用 `AuthRepository` 呼叫 `POST /api/v1/auth/login`；不得呼叫 legacy account migration endpoint。成功後由 `ApiClient.setAccessToken()` 將 access token 套用到目前執行階段的 Authorization header。使用者再次開啟App時，`SplashPage`會以暫存Credential呼叫`POST /api/v1/auth/session-refreshes`完成opaque rotation，再使用新access token進入首頁；不得直接重用可能已過期的access token。Task 08仍須將暫時`AuthSessionStore`改為Web Cookie及App Keychain／Keystore並移除SharedPreferences Token。
